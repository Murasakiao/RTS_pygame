...
Known gaps: mixed/circular imports create duplicate entity classes; terrain regeneration keeps the same seed and a stale render reference; navigation permits corner cutting and obstacle-bypassing movement; placement/spawning and building attack range need repair. Runtime and development dependency manifests now pin the verified macOS environment, but no committed test source, save/load, or win/lose state exists. Code and art licensing need confirmation.
...
## Conventions
- Launch from the unit root with `python src/rts.py` using the project `venv`.
- `python -m src.rts` fails with the current imports; relative asset paths require the root working directory.
