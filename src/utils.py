...
def spawn_enemies(buildings, units, current_wave, enemy_spawn_rate, mini_bosses=None): # Updated to handle mini-bosses correctly
    ...
    # --- Enemy Compositions ---
    enemy_compositions = [
        ["Goblin", "Goblin"],  # Basic melee
        ["GoblinArcher", "GoblinArcher"],  # Basic ranged
        ["Goblin", "Goblin", "GoblinArcher"],  # Mixed
        ["Orc", "Goblin"], # Stronger melee with basic melee
    ]

    # Calculate the number of enemies based on a more complex formula
    num_enemies = int(enemy_spawn_rate * (current_wave ** 1.2)) # Increased scaling

    for _ in range(num_enemies):
        spawn_x, spawn_y = generate_spawn_point()
        composition = random.choice(enemy_compositions)
        for enemy_type in composition:
            enemy = EnemyUnit(enemy_type, spawn_x, spawn_y, buildings, units)
            spawned_enemies.append(enemy)
            print(f"Wave {current_wave}: Spawning enemy: {enemy_type}")
...
def manage_waves(wave_timer, current_wave, dt, buildings, units, enemies, enemy_spawn_rate, wave_interval=WAVE_INTERVAL, mini_bosses=None): # Added mini_bosses parameter
    ...
    if wave_timer >= wave_interval and not wave_in_progress:
        ...
        wave_start_time = pygame.time.get_ticks() # Record the start time
        print(f"Starting wave {current_wave}") # Debug message
        add_game_message(f"Wave {current_wave} Incoming!", game_messages, duration=5000) # Display message
