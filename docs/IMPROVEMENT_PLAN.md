...
| B07 | Partly resolved in P1 | Shared `World.validate_footprint()` checks full bounds, terrain, buildings, units, and enemies; shared atomic affordability/deduction now covers costs; lane sealing, placement transaction unification, and map connectivity remain | `world.py`, `game.py`, placement handler and preview helpers | P1, P3 |
...
- [x] Add `World.validate_footprint()` plus valid trainer exits and free edge spawn-cell selection. Shared `can_afford()`/`deduct_cost()` helpers now validate and apply costs atomically; successful construction updates costs and navigation together. Lane/connectivity transactions remain for later validation.
