...
- [x] Add `World.validate_footprint()` plus valid trainer exits and target-reachable free edge spawn-cell selection. Shared `can_afford()`/`deduct_cost()` helpers now validate and apply costs atomically; successful construction updates costs and navigation together. Lane/connectivity transactions and pending spawn accounting remain for later validation.
